...
// ================== INIT ==================
fetchFilms();
function showFilmStats() {
  const query = document
    .getElementById('statsSearchInput')
    .value.trim()
    .toLowerCase();
  const statsContainer = document.getElementById('filmStatsResult');

  if (!query) {
    statsContainer.innerHTML = `<p>❗ Hãy nhập tên phim để xem thống kê.</p>`;
    return;
  }

  // Lọc phim theo tên
  const matchedFilms = dashboardData.movies.filter(f =>
    f.ten_phim.toLowerCase().includes(query),
  );

  if (!matchedFilms.length) {
    statsContainer.innerHTML = `<p>❌ Không tìm thấy phim nào liên quan đến "${query}".</p>`;
    return;
  }

  // Tính thống kê cho từng phim
  statsContainer.innerHTML = matchedFilms
    .map(film => {
      // Lấy tất cả vé của phim này
      const filmTickets = dashboardData.tickets.filter(
        t => t.ten_phim === film.ten_phim,
      );

      // Doanh thu từ các payment liên quan
      let revenue = 0;
      filmTickets.forEach(ticket => {
        const pay = dashboardData.payments.find(p => p.ve_id === ticket.ve_id);
        if (pay) revenue += Number.parseFloat(pay.so_tien) || 0;
      });

      return `
        <div class="film-stats-item">
          <h3>${film.ten_phim}</h3>
          ${
            film.hinh_anh
              ? `<img src="${film.hinh_anh}" alt="${film.ten_phim}" style="max-width:200px;border-radius:12px;margin:8px 0;">`
              : ''
          }
          <p><strong>Đạo diễn:</strong> ${film.dao_dien || 'Chưa rõ'}</p>
          <p><strong>Ngôn ngữ:</strong> ${film.ngon_ngu || 'Chưa rõ'}</p>
          <p><strong>Độ tuổi:</strong> ${film.do_tuoi || '-'}+</p>
          <p><strong>Thời lượng:</strong> ${film.thoi_luong || '-'} phút</p>
          <p><strong>Thể loại:</strong> ${film.the_loai || 'Chưa rõ'}</p>
          <hr>
          <p><strong>📈 Doanh thu:</strong> ${formatCurrency(revenue)}</p>
          <p><strong>🎟️ Vé bán ra:</strong> ${filmTickets.length}</p>
        </div>
      `;
    })
    .join('');
